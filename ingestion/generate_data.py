...
customers = pd.DataFrame({
    ...
    'signup_date': [(datetime(2023, 1, 1) + timedelta(days=random.randint(0, 365))).strftime('%Y-%m-%d') for _ in range(500)]
})
customers.to_csv(f'{OUTPUT_DIR}/customers.csv', index=False)
print(f'customers.csv done - {len(customers)} rows')
...
print(f'products.csv done - {len(products)} rows')
...
orders = pd.DataFrame({
    ...
    'created_at':  [(datetime(2024, 1, 1) + timedelta(days=random.randint(0, 364))).strftime('%Y-%m-%d %H:%M:%S') for _ in range(5000)]
})
orders.to_csv(f'{OUTPUT_DIR}/orders.csv', index=False)
print(f'orders.csv done - {len(orders)} rows')
...
inventory = pd.DataFrame({
    ...
    'updated_at':   [(datetime(2024, 6, 1) + timedelta(hours=random.randint(0, 720))).strftime('%Y-%m-%d %H:%M:%S') for _ in range(100)]
})
inventory.to_csv(f'{OUTPUT_DIR}/inventory.csv', index=False)
print(f'inventory.csv done - {len(inventory)} rows')
